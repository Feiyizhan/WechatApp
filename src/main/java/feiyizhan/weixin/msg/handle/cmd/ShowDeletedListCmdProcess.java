...
public class ShowDeletedListCmdProcess extends BaseCmdProcess {
		...
		if("获取删除我的用户清单".equals(cmd)){
			if(!this.getHandle().getControl().isBatchFlag()){
				new Thread(){
					 ...
					 */
					public void run(){
						List<String> list = getDeletedMeUserListWithCreatRoom();
	 ...
	 * @return
	 */
	public ArrayList<String> getDeletedMeUserListWithCreatRoom(){
		...
		for(JSONValue val:this.getHandle().getSession().ContactList){
			...
			LOGGER.info("[*]"+id +"|"+UserUtil.getUserRemarkName(obj));
			count ++ ;
			if (count>=20){
